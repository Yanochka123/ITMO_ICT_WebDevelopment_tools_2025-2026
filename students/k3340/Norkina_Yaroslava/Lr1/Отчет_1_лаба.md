...
Self-referencing (Подзадачи): При запросе задачи, ответ включает список subtasks с их деталями.

Что такое remote_side и зачем он нужен
В self-referential связи (задача → задача) SQLAlchemy нужно понять, какая сторона «родитель», а какая «ребёнок». Обе стороны ссылаются на один и тот же класс Task и используют одно и то же FK-поле parent_task_id.

subtasks — это «дочерняя» сторона (one-to-many). Здесь remote_side не нужен.

parent_task — это «родительская» сторона (many-to-one). Здесь remote_side=[id] нужен, чтобы SQLAlchemy знал: «удалённая» сторона — это id родителя, а локальная — parent_task_id.

В вашей модели remote_side отсутствует, но это не ошибка, потому что:

foreign_keys=[parent_task_id] уже явно указывает, по какому полю строить связь.

backref="parent_task" автоматически создаёт обратную связь, и SQLAlchemy сам выводит remote_side из контекста foreign_keys.

backref="parent_task" создаёт обратную связь. Но по умолчанию backref наследует тот же direction, что и subtasks (то есть ONETOMANY). Получается, что и subtasks, и parent_task — обе ONETOMANY, что логически невозможно.

SQLAlchemy прямо намекает: «Did you mean to set remote_side on the many-to-one side?» — то есть добавьте remote_side=[id] на сторону parent_task, чтобы она стала MANYTOONE.

Решение
Нужно явно описать обе стороны связи, а не использовать backref (потому что backref не даёт указать remote_side для обратной стороны).
...
Это реализовано через модели Pydantic (например, TaskResponse), которые используют from_attributes = True для сериализации связанных SQLAlchemy-объектов.
...
### Тестирование запросов и функциоальности.
