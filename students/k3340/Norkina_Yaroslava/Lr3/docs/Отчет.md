...
Код выполненных работ:
1.1  https://github.com/Yanochka123/ITMO_ICT_WebDevelopment_tools_2025-2026/commit/c543331d54e14eb58a6d12bfd3dde27a55d252e9
1.2  https://github.com/Yanochka123/ITMO_ICT_WebDevelopment_tools_2025-2026/commit/e0a875cc648c32296250706ed64a53b190c2844d
1.3  https://github.com/Yanochka123/ITMO_ICT_WebDevelopment_tools_2025-2026/commit/3fbc68fac8963b1bc9552dfd60dceb9a943693c2
...
Интеграция Alembic в разрабатываемый проект:
```Python
pip install alembic
```
...
```
После этого в корне проекта, помимо ранее созданных файлов должна сформироваться следующая структура:
migratgions/
...
<img width="413" height="270" alt="image" src="https://github.com/user-attachments/assets/90f3c4bd-92aa-4b14-bf36-c864d2c39f74" />

Далее указываем адрес БД в файле настоек миграций (sqlalchemy.url). Драйвер для данного проекта - postgresql+psycopg2
В файле env.py настраиваем импорт models.py и в переменной target_metadata указываем значение target_metadata=SQLModel.metadata
...
```
В файле script.py.mako импортирована библиотека sqlmodel.
Изменим модель и таблицу в БД соответственно. Добавим новое поле модели SkillWarriorLink:
```python
class SkillWarriorLink(SQLModel, table=True):
    skill_id: Optional[int] = Field(
    default=None, foreign_key="skill.id", primary_key=True
    ...
    level: int | None
```
Создан файл миграций с помощью команды ```alembic revision --autogenerate -m "skill added"```
Команда выполнена успешно:
<img width="1331" height="367" alt="image" src="https://github.com/user-attachments/assets/07e95bf7-a503-46d2-bdef-8c68989845fc" />

Применение миграции происходит через команду ```alembic upgrade head ```
Навык добавлен:
<img width="1339" height="202" alt="image" src="https://github.com/user-attachments/assets/d36bf1f0-788c-4bcb-b8f5-0dc9da49a6c1" />

После успешно примененных миграций был добавлен файл .env для сохранения пароля с доступом к БД локально.
...
А также в финальной версии проекта настроен файл .gitignore для того, чтобы переменные из виртуального окружения или пароль для разработки не попадали в открытый доступ.

```python
# Игнорируем конкретные папки внутри time-management-app
time-management-app/project/
time-management-app/app/
time-management-app/static/

# Игнорируем виртуальное окружение где угодно
f_api_env/
venv/
env/

# Игнорируем кэш Python где угодно
.__pycache__/
*.pyc

# Игнорируем файлы .env где угодно
*.env
.env

# Игнорируем папку IDE
.idea/
.vscode/
```

### Вывод по практическим работам 1-3.
В ходе выполнения работ были получены навыки по настройке и написанию базовых АПИ-эндпойнтов для сервисного приложения на FastApi. Получилось правильно организовать код и файловую структуру, настроить отображение документации к написанным методам, запустить рабочее приложение при помощи веб-сервиса uvicorn, а также разобраться в темах аннотирования данных при помощи Pydantic, настройки и работы с SQLAlchemy и SQLModel. Также была создана реальная база данных через средства работы с PostgreSQL, успешно проведены миграции для изменения структуры уже существующих таблиц в ней и настроено безопасное хранение данных для последующей выгрузки их в репозиторий на гитхаб.